#!/usr/bin/env bash
# Phase 2 LAFA-band re-scoring chain (goa:226 -> goa:230).
#
# For each of the 24 (PLM x K) v226-lineage cells:
#  1. Skip predict if a canonical v226 prediction_set on the LAFA query_set
#     (045ab275, 10609 delta proteins) already exists.
#  2. Otherwise dispatch predict_go_terms with search_backend=numpy
#     (torch backend OOMs on the 10609x500k corpus, dead-letters after 5
#     retries; smoke succeeded only on the 2-protein fake query_set).
...
#     run_cafa_evaluation against eval_set 3b6f8064. Each call writes
...
LOG=/home/frapercan/Thesis2/phase2-lafa-chain.log
exec >>"$LOG" 2>&1

ts() { date -u +%Y-%m-%dT%H:%M:%SZ; }
log() { echo "[$(ts)] $*" >&2; }

log "=== Phase 2 LAFA chain started (numpy backend, 7 scoring_configs) ==="

TOKEN=$(poetry run python -c "
import jwt, time, os
print(jwt.encode({
    'sub': 'conductor-phase2-lafa',
    'iat': int(time.time()),
...
# Canonical IDs shared across every cell.
QUERY_SET=045ab275-97de-4793-81dd-bcbd37997544          # eval-3b6f8064-delta-10609
ANN_SET=5e1a7285-bb85-4816-88ab-2a32f7e96dd5            # v226 train pool
ONT=35c3ad67-3002-47db-8f71-eeed69d22ad6                 # GO snapshot
EVAL_SET=3b6f8064-239c-4af4-8390-fbb0f4c59914            # goa:226 -> goa:230
...
# (k|embedding_config_id|plm_label)  33 v226-lineage cells:
# 24 canonical (ADR D35) + 9 orphan PLMs added 2026-05-30 (esmc_300m,
# ankh_base_v2 = c2868c1a config duplicate, prot_t5_full = full
# precision variant). Orphans have full ~527k embeddings but were
# never benchmarked; including them gives an 11-PLM LAFA grid.
CELLS=(
  ...
  "3|c85d1afe-3f49-4ead-82d9-faaa6efe7a2c|esmc_300m"
  "3|c2868c1a-0966-47a0-9941-2213fe6d22fa|ankh_base_v2"
  "3|db4db5ed-e34a-47af-a9ab-cc0f230b0a8c|prot_t5_full"
  ...
  "5|c85d1afe-3f49-4ead-82d9-faaa6efe7a2c|esmc_300m"
  "5|c2868c1a-0966-47a0-9941-2213fe6d22fa|ankh_base_v2"
  "5|db4db5ed-e34a-47af-a9ab-cc0f230b0a8c|prot_t5_full"
  ...
  "10|c85d1afe-3f49-4ead-82d9-faaa6efe7a2c|esmc_300m"
  "10|c2868c1a-0966-47a0-9941-2213fe6d22fa|ankh_base_v2"
  "10|db4db5ed-e34a-47af-a9ab-cc0f230b0a8c|prot_t5_full"
)
...
#
# Threshold-based version (v1, 2026-05-30 morning) marked SUCCEEDED at
# 250k preds — fine for K=3 (~319k expected) but premature for K=10
# (~700k expected); early scorings then computed Fmax over partial
# data. Stability-based v2 (this version) is K-agnostic.
...
log "=== Phase 2 LAFA chain complete: 24 cells x 7 scoring_configs scored on 226->230 ==="
